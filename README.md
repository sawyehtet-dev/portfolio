...
- No form library: the contact form is plain React state over native HTML
  constraints, posting to Formspree
...
- **`src/config/editorial-data.ts`**: `PROJECTS`, `EXPERIENCE`, `STATS`,
  `TESTIMONIALS`, `SKILL_LANES`, and `SKILL_BANDS`.
...
color (`#a8432b`), a warm paper background, no glassmorphism or dark-card defaults.
...
- **CSP `style-src 'unsafe-inline'`**: required by React inline `style` props. An
  accepted trade-off.
- **Contact form**: an off-screen honeypot field plus client-side validation on the
  trimmed values, backed by the native HTML constraints on each input. Submissions
  go through Formspree (the form ID is public by design).
...
The script palette-quantizes the screenshot to 256 colours itself (via ImageMagick,
skipped with a note if it isn't installed), so there is no manual optimize step
before committing - the card is flat editorial art, so this is visually lossless at
